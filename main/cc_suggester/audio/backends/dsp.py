...
def _read_energy_windows(
    ...
    *,
    ...
    window_bytes = max(1, int(sample_rate * window_seconds) * sample_width)
    hop_bytes = max(1, int(sample_rate * hop_seconds) * sample_width)
    max_amplitude = float(2 ** (8 * sample_width - 1))

    windows: list[EnergyWindow] = []
    for start_byte in range(0, max(0, len(frames) - window_bytes + 1), hop_bytes):
        chunk = frames[start_byte : start_byte + window_bytes]
        if len(chunk) < window_bytes:
            break
        start = start_byte / sample_width / sample_rate
